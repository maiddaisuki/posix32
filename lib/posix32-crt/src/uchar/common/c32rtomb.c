...
static void P32LocaleFunction_c32rtomb (LocaleFunctions *functions, Charset *charset) {
  if (charset->CodePage == P32_CODEPAGE_ASCII) {
    functions->F_c32rtomb = p32_private_c32rtomb_cp20127;
  } else if (charset->CodePage == P32_CODEPAGE_ISO_8859_1) {
    functions->F_c32rtomb = p32_private_c32rtomb_cp28591;
  } else if (charset->CodePage == CP_UTF8) {
    functions->F_c32rtomb = p32_private_c32rtomb_cp65001;
  ...
  assert (functions->F_c32rtomb != NULL);
}

size_t p32_private_c32rtomb_l (char *u8str, char32_t u32char, mbstate_t *state, locale_t locale) {
  assert (state != NULL);
...
size_t p32_c32rtomb_l (char *u8str, char32_t u32char, mbstate_t *state, locale_t locale) {
  ...
  return p32_private_c32rtomb_l (u8str, u32char, state, locale);
}
