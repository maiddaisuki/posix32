 ...
 * Test Summary:
 *
 * Test `strchr` function with a DBCS code page.
 */
...
static void DoTest (void) {
  ...
  for (int i = 1; i < 0x80; ++i) {
    assert (strchr (text, i) == NULL);
  }

  assert (strchr (text, '\0') == text + 16);
  assert (strchr (text, '\n') == NULL);
  ...
  assert (strchr (text, '|') == text + 2);

  assert (strchr (text, '\0') == text + 8);
  assert (strchr (text, '\n') == NULL);
  ...
  assert (strchr (invalid_text, '|') == invalid_text + 2);
  assert (strchr (invalid_text, '\0') == NULL);
