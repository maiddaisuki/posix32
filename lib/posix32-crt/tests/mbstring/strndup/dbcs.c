 ...
 * Test Summary:
 *
 * Test `p32_private_strndup_l` function with DBCS code page 932.
 */
...
static void DoTest (void) {
  ...
  assert (str == NULL || strcmp (str, STRING (C (0x82), C (0xA1))) == 0);
  free (str);

  _set_errno (0);
  assert ((str = strndup (Test3String, 3)) != NULL || errno == ENOMEM);
  assert (str == NULL || strcmp (str, STRING (C (0x82), C (0xA1), C (0xA1))) == 0);
  free (str);
  ...
  _set_errno (0);
  assert ((str = strndup (Test4String, 2)) != NULL || errno == ENOMEM);
  assert (str == NULL || strcmp (str, STRING (C (0x82), C (0xA1))) == 0);
  free (str);

  _set_errno (0);
  assert ((str = strndup (Test4String, 3)) != NULL || errno == ENOMEM);
  assert (str == NULL || strcmp (str, STRING (C (0x82), C (0xA1))) == 0);
  free (str);
  ...
  _set_errno (0);
  assert ((str = strndup (Test5String, 4)) == NULL);
  assert (errno != 0 && errno != ENOMEM);
  free (str);
