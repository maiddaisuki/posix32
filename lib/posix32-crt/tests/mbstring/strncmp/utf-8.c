 ...
 * Test Summary:
 *
 * Test `p32_private_strncmp_l` function with UTF-8 (code page 65001).
 */
...
static void DoTest (void) {
  ...
  for (char32_t c32 = 0x01; c32 <= 0x0010FFFF; ++c32) {
    ...
    for (size_t i = 1; i < length; ++i) {
       ...
       */
      assert ((diff = strncmp ("", Char, i)) != _NLSCMPERROR);
      assert (diff < 0);
      assert ((diff = strncmp (Char, "", i)) != _NLSCMPERROR);
      assert (diff > 0);
       ...
       */
      if (i == PrevLength) {
        assert ((diff = strncmp (Prev, Char, i)) != _NLSCMPERROR);
        assert (diff > 0);
        assert ((diff = strncmp (Char, Prev, i)) != _NLSCMPERROR);
        assert (diff < 0);
        continue;
      }
    ...
    assert ((diff = strncmp (Prev, Char, SIZE_MAX)) != _NLSCMPERROR);
    assert (diff < 0);
    assert ((diff = strncmp (Char, Prev, SIZE_MAX)) != _NLSCMPERROR);
    assert (diff > 0);
