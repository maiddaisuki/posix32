 ...
 * Test Summary:
 *
 * Test `p32_private_strchr_l` function with SBCS code page 1253.
 *
...
static void DoTest (void) {
  ...
  assert (strchr (Test2String, 'D') == NULL);
  assert (strchr (Test2String, 'F') == NULL);
  assert (strchr (Test2String, C8 ('A')) == Test2String + 1);
  assert (strchr (Test2String, C8 ('F')) == NULL);
  assert (strchr (Test2String, 0xAA) == NULL);
  assert (strchr (Test2String, 0xD2) == NULL);
  assert (strchr (Test2String, 0xFF) == NULL);
  assert (strchr (Test2String, '\0') == NULL);
}
