 ...
 * Test Summary:
 *
 * Test `p32_private_strncpy_l` function with DBCS code page 932.
 */
...
static void DoTest (void) {
  ...
  memset (buffer, 0xFF, BUFSIZ);
  assert (strncpy (buffer, Test4String, 2) == buffer);
  assert (strncmp (buffer, Test4String, 2) == 0);
  assert (buffer[2] == C (0xFF));

  memset (buffer, 0xFF, BUFSIZ);
  assert (strncpy (buffer, Test4String, 3) == buffer);
  assert (strncmp (buffer, Test4String, 2) == 0);
  assert (buffer[2] == '\0');
  assert (buffer[3] == C (0xFF));
  ...
  memset (buffer, 0xFF, BUFSIZ);
  assert (strncpy (buffer, Test5String, 4) == NULL);
