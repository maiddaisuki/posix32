 ...
 * Test Summary:
 *
 * Test `p32_private_strtok_r_l` function with ISO-8859-1 (code page 28591).
 *
...
static void DoTest (void) {
   ...
   */
  const char *Test5String = STRING (C (0x80), C (0x90), C (0xA0), C (0xB0), C (0xC0), C (0xD0), C (0xE0), C (0xF0));
  const char *Test5Set    = STRING (C (0xA0), C (0xD0));

  assert ((str = strdup (Test5String)) != NULL);

  assert ((token = strtok_r (str, Test5Set, &context)) == str);
  assert (strcmp (token, STRING (C (0x80), C (0x90))) == 0);
  assert (context == str + 3);

  assert ((token = strtok_r (NULL, Test5Set, &context)) == str + 3);
  assert (strcmp (token, STRING (C (0xB0), C (0xC0))) == 0);
  assert (context == str + 6);

  assert ((token = strtok_r (NULL, Test5Set, &context)) == str + 6);
  assert (strcmp (token, STRING (C (0xE0), C (0xF0))) == 0);
  assert (context == str + 8);
