 ...
 * Test Summary:
 *
 * Test `p32_private_strncat_l` function with ASCII (code page 20127).
 */
...
static void DoTest (void) {
  ...
  UNREFERENCED_PARAMETER (Test2Result);

  memset (buffer, 0xFF, BUFSIZ);
  strcpy (buffer, Test2Base);
  assert (strncat (buffer, "", 0) == NULL);
  assert (strcmp (buffer, Test2Base) == 0);
  assert (buffer[4] == C (0xFF));

  memset (buffer, 0xFF, BUFSIZ);
  strcpy (buffer, Test2Base);
  assert (strncat (buffer, Test2String, SIZE_MAX) == NULL);
  assert (strcmp (buffer, Test2Base) == 0);
  assert (buffer[4] == C (0xFF));
}
