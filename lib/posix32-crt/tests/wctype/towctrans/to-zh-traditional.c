 ...
 * Test Summary:
 *
 * Test "to-zh-traditional" character mapping.
 *
 * This character mapping translates simplified Chinese logographics to
 * thier traditional counterparts.
 *
...
static bool __cdecl Test (locale_t locale, const wchar_t *localeName) {
  wctrans_t to_zh_traditional = (wctrans_t) 0;

  assert ((to_zh_traditional = wctrans_l ("to-zh-traditional", locale)) != (wctrans_t) 0);

  for (wchar_t wc = 0;; ++wc) {
    wchar_t wcTranslated = towctrans_l (wc, to_zh_traditional, locale);
