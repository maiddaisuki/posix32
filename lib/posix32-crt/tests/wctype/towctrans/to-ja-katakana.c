 ...
 * Test Summary:
 *
 * Test "to-ja-katakana" character mapping.
 *
 * This character mapping translates Japanese Hiragana to Katakana.
 *
...
static bool __cdecl Test (locale_t locale, const wchar_t *localeName) {
  wctrans_t tokatakana = (wctrans_t) 0;

  assert ((tokatakana = wctrans_l ("to-ja-katakana", locale)) != (wctrans_t) 0);

  for (wchar_t wc = 0;; ++wc) {
    wchar_t wcTranslated = towctrans_l (wc, tokatakana, locale);
