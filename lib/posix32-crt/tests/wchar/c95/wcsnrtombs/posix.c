 ...
 * Test Summary:
 *
 * Test `wcsnrtombs` function with ISO-8859-1 (code page 28591).
 *
...
/**
 * Convenience macro to call `p32_private_wcsnrtombs_cp28591`.
 */
#define wcsnrtombs(mbs, wcs, cnt, sz, state) p32_private_wcsnrtombs_cp28591 (mbs, wcs, cnt, sz, state, &iso_8859_1)
...
static void DoTest (void) {
  ...
  /**
   * Get length of 5 wide characters from `text` converted
   *
   ...
   */
  assert (wcsnrtombs (NULL, &text, 5, 0, &state) == 5);
  assert (text == original_text);
  assert (mbsinit (&state));
  assert (errno == 0);

  /**
   * Convert at most 5 wide characters from `text`
   *
   * - return value must be 5
   ...
   * - `errno` must not change
   */
  assert (wcsnrtombs (buffer, &text, 5, BUFSIZ, &state) == 5);
  ...
  /**
   * Convert at most 10 wide characters from `text` while writing at most
   * 5 bytes to `buffer`
   *
   * - return value must be 5
   ...
   * - `errno` must not change
   */
  assert (wcsnrtombs (&buffer[5], &text, 10, 5, &state) == 5);
   ...
   * - `errno` must not change
   */
  assert (wcsnrtombs (NULL, &text, INT_MAX, 0, &state) == 5);
  assert (text == original_text + 10);
  assert (mbsinit (&state));
  assert (errno == 0);
   ...
   * - `errno` must not change
   */
  assert (wcsnrtombs (&buffer[10], &text, INT_MAX, BUFSIZ, &state) == 5);
  assert (text == NULL);
  assert (buffer[15] == '\0' && buffer[16] == EOF);
   ...
   */
  assert (wcsnrtombs (buffer, &text, 1, 0, &state) == (size_t) -1);
  assert (text == original_text);
  assert (buffer[0] == EOF);
  assert (mbsinit (&state));
  assert (errno == EILSEQ);
