 ...
 * Test Summary:
 *
 * Test `mbrtowc` function with ASCII (code page 20127).
 */
...
/**
 * Convenience macro to call `p32_private_mbrtowc_cp20127`.
 */
#define mbrtowc(wc, mb, count, state) p32_private_mbrtowc_cp20127 (wc, mb, count, state, &ascii)
...
static void DoTest (void) {
  ...
  for (uint8_t c = 128;; ++c) {
    ...
    assert (mbrtowc (&wc, (char *) &c, MB_CUR_MAX, &state) == (size_t) -1);
    assert (wc == WEOF);
    assert (mbsinit (&state));
    assert (errno == EILSEQ);
