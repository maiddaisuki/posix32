 ...
 * Test Summary:
 *
 * Test `mbrtowc` function with some DBCS code page.
 *
...
/**
 * Convenience macro to call `p32_private_mbrtowc_dbcs`.
 */
#define mbrtowc(wc, mb, count, state)            p32_private_mbrtowc_dbcs (wc, mb, count, state, &cp932)
#define mbrtowc_l(wc, mb, count, state, charset) p32_private_mbrtowc_dbcs (wc, mb, count, state, &charset)
...
static void DoTest (void) {
  ...
  for (size_t i = 0; i < _countof (CJK); ++i) {
    ...
    assert (mbrtowc (&wc, CJK[i].DBCS, MB_CUR_MAX, &state) == 2);
    assert (wc == CJK[i].Wc);
    assert (mbsinit (&state));
    assert (errno == 0);
    ...
    assert (mbrtowc (&wc, &CJK[i].DBCS[0], 1, &state) == (size_t) -2);
    assert (wc == WEOF);
    assert (!mbsinit (&state));
    assert (errno == 0);

    assert (mbrtowc (&wc, &CJK[i].DBCS[1], 1, &state) == 1);
    assert (wc == CJK[i].Wc);
    assert (mbsinit (&state));
    assert (errno == 0);
    ...
    assert (mbrtowc (&wc, InvalidDBCS, MB_CUR_MAX, &state) == (size_t) -1);
    assert (wc == WEOF);
    assert (mbsinit (&state));
    assert (errno == EILSEQ);
   ...
   */
  assert (mbrtowc (&wc, &CJK[0].DBCS[0], 1, &state) == (size_t) -2);
  assert (wc == WEOF);
  assert (!mbsinit (&state));
  assert (errno == 0);
   ...
   */
  assert (mbrtowc (&wc, &CJK[0].DBCS[1], 0, &state) == (size_t) -2);
  assert (wc == WEOF);
  assert (!mbsinit (&state));
  assert (errno == 0);
   ...
   */
  assert (mbrtowc (&wc, NULL, 0, &state) == (size_t) -1);
  assert (wc == WEOF);
  assert (!mbsinit (&state));
  assert (errno == EILSEQ);
   ...
   */
  assert (mbrtowc_l (&wc, "", 0, &state, dbcs) == (size_t) -1);
  assert (wc == WEOF);
  assert (!mbsinit (&state));
  assert (errno == EINVAL);
   ...
   * Complete conversion.
   */
  assert (mbrtowc (&wc, &CJK[0].DBCS[1], 1, &state) == 1);
  assert (wc == CJK[0].Wc);
  assert (mbsinit (&state));
  assert (errno == 0);
}
