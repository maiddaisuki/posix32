...
static void DoTest (void) {
  ...
  /**
   * When second argument to `mbrtoc16` is zero, it must not examine passed
  ...
  for (size_t i = 0; i < _countof (CJK); ++i) {
    /**
     * Get length of DBCS character with single call.
     */
    assert (mbrlen (CJK[i].DBCS, MB_CUR_MAX, &state) == 2);
    assert (mbsinit (&state));
    assert (errno == 0);

    /**
     * Get length of DBCS character with multiple calls.
     */
    assert (mbrlen (&CJK[i].DBCS[0], 1, &state) == (size_t) -2);
    assert (!mbsinit (&state));
    assert (errno == 0);

    assert (mbrlen (&CJK[i].DBCS[1], 1, &state) == 1);
    assert (mbsinit (&state));
    assert (errno == 0);
    ...
    assert (mbrlen (InvalidDBCS, MB_CUR_MAX, &state) == (size_t) -1);
    assert (mbsinit (&state));
    assert (errno == EILSEQ);
   ...
   */
  assert (mbrlen (&CJK[0].DBCS[0], 1, &state) == (size_t) -2);
  assert (!mbsinit (&state));
  assert (errno == 0);

  /**
   * Call with `0` for second argument.
   */
  assert (mbrlen (&CJK[0].DBCS[1], 0, &state) == (size_t) -2);
  assert (!mbsinit (&state));
  assert (errno == 0);

  /**
   * Call with `NULL` for first argument.
   */
  assert (mbrlen (NULL, 0, &state) == (size_t) -1);
  assert (!mbsinit (&state));
  assert (errno == EILSEQ);
   ...
   * Complete conversion.
   */
  assert (mbrlen (&CJK[0].DBCS[1], 1, &state) == 1);
  assert (mbsinit (&state));
  assert (errno == 0);
}
