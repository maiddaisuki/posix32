 ...
 * Test Summary:
 *
 * Test `mbrlen` function with ASCII (code page 20127).
 */
...
/**
 * Convenience macro to call `p32_private_mbrlen_cp20127`.
 */
#define mbrlen(mb, count, state) p32_private_mbrlen_cp20127 (mb, count, state, &ascii)
...
static void DoTest (void) {
   ...
   */
  for (uint8_t c = 128;; ++c) {
    assert (mbrlen ((char *) &c, MB_CUR_MAX, &state) == (size_t) -1);
    assert (mbsinit (&state));
    assert (errno == EILSEQ);
