 ...
 * Test Summary:
 *
 * Test `wcsrtombs` function with some SBCS code page.
 *
...
/**
 * Convenience macro to call `p32_private_wcsrtombs_sbcs`.
 */
#define wcsrtombs(mbs, wcs, size, state) p32_private_wcsrtombs_sbcs (mbs, wcs, size, state, &cp1252)
...
static void DoTest (void) {
   ...
   * - `errno` must not change
   */
  assert (wcsrtombs (&buffer[10], &text, BUFSIZ, &state) == 5);
  assert (text == NULL);
