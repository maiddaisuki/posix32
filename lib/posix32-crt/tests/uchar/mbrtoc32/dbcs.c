 ...
 * Test Summary:
 *
 * Test `mbrtoc32` function with some DBCS code page.
 *
...
/**
 * Convenience macros to call `p32_private_mbrtoc32_dbcs`.
 */
#define mbrtoc32(c32, mb, count, state)            p32_private_mbrtoc32_dbcs (c32, mb, count, state, &cp932)
#define mbrtoc32_l(c32, mb, count, state, charset) p32_private_mbrtoc32_dbcs (c32, mb, count, state, &charset)
...
static void DoTest (void) {
   ...
   */
  assert (mbrtoc32_l (&u32, "", 0, &state, dbcs) == (size_t) -1);
  assert (u32 == 0xFFFFFFFF);
  assert (!mbsinit (&state));
  assert (errno == EINVAL);
