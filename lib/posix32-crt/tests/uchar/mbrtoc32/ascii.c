 ...
 * Test Summary:
 *
 * Test `mbrtoc32` function with ASCII (code page 20127).
 */
...
/**
 * Convenience macro to call `p32_private_mbrtoc32_cp20127`.
 */
#define mbrtoc32(c32, mb, count, state) p32_private_mbrtoc32_cp20127 (c32, mb, count, state, &ascii)
...
static void DoTest (void) {
  ...
  for (uint8_t c = 0x80;; ++c) {
    ...
    assert (mbrtoc32 (&u32, (char *) &c, MB_CUR_MAX, &state) == (size_t) -1);
    assert (u32 == 0xFFFFFFFF);
    assert (mbsinit (&state));
    assert (errno == EILSEQ);
